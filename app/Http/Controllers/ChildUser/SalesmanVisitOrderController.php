...
class SalesmanVisitOrderController extends Controller
{
    ...
    public function orderForms(Request $request)
    {
        ...
        return view('child-user.business-order.order-forms',
    ...
    public function returnOrders(Request $request)
    {
        ...
        return view('child-user.business-order.return-orders',
            ['orders' => $orders, 'salesmen' => $salesmen, 'data' => $data]);
    }
    ...
    public function browserExport(SalesmanVisitOrder $salesmanVisitOrder)
    {
        ...
        return view('child-user.business-order.print', (new BusinessService)->getOrderData($salesmanVisitOrder));
    }
    ...
    {
        if (Gate::forUser(child_auth()->user())->denies('validate-salesman-order',
        ...
        if ($isOrderForm) {
            ...
            $viewName = 'return-detail';
            $load = ['mortgageGoods.goods', 'orderGoods.goods'];
        }

        // $viewName = $isOrderForm ? 'order-return-detail' : 'order-detail';

        return view('child-user.business-order.' . $viewName, (new BusinessService)->getOrderData($salesmanVisitOrder, $load));
    }
