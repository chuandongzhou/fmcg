...
class InfoController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($type == cons('user.type.retailer')) {
            //订单统计
            $countData = Order::select(DB::raw('count(if(status >= ' . $status['non_send'] . ' and status < ' . $status['finished'] . ' and pay_status=' . $payStatus['non_payment'] . ' and pay_type <> ' . $payType['pick_up'] . ',true,null)) AS waitReceive,count(if(((pay_type=' . $payType['online'] . ' and pay_status=' . $payStatus['payment_success'] . ') or pay_type=' . $payType['cod'] . ') and status=' . $status['non_send'] . ',true,null)) as waitSend,count(if(status=' . $status['send'] . ',true,null)) as refund,count(if(status=' . $status['non_confirm'] . ',true,null)) as waitConfirm'))->ofBuy(auth()->id())->nonCancel()->first();
            //本月销售图表信息
