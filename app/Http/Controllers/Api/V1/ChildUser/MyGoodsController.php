...
class MyGoodsController extends Controller
{
    ...
    public function store(Requests\Api\v1\CreateGoodsRequest $request)
    {
        ...
        $result = DB::transaction(function () use ($user, $request) {
            ...
            $attributes['user_type'] = child_auth()->user()->type;
    ...
    {
        if (Gate::forUser(child_auth()->user())->denies('validate-my-goods', $goods)) {
    ...
    public function batchShelve(Request $request)
    {
        ...
        $goods = Goods::where('shop_id', child_auth()->user()->shop()->pluck('id'))->whereIn('id', $goodsIds)->get();
    ...
    public function importGoods($file, $postAttr, $attrs)
    {
        ...
        if ($shopId) {
            ...
            $shop = child_auth()->user()->shop->load(['deliveryArea']);
        }
        if (is_null($shop)) {
