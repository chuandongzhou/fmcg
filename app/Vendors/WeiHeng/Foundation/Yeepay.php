...
function getReqHmacString($p2_Order,$p3_Amt,$p4_Cur,$p5_Pid,$p6_Pcat,$p7_Pdesc,$p8_Url,$pa_MP,$pd_FrpId,$pr_NeedResponse)
{
    ...
    $sbOld = "";
    #加入业务类型
    $sbOld = $sbOld.config('yeepay.p0_cmd');
    #加入商户编号
    $sbOld = $sbOld.config('yeepay.p1_mer_id');
    #加入商户订单号
    ...
    $sbOld = $sbOld.$p8_Url;
    #加入送货地址标识
    $sbOld = $sbOld.config('yeepay.p9_saf');
    #加入商户扩展信息
    $sbOld = $sbOld.$pa_MP;
    #加入支付通道编码
    ...
    logstr($p2_Order,$sbOld,HmacMd5($sbOld,config('yeepay.merchant_key')));
    return HmacMd5($sbOld,config('yeepay.merchant_key'));
...
function getCallbackHmacString($r0_Cmd,$r1_Code,$r2_TrxId,$r3_Amt,$r4_Cur,$r5_Pid,$r6_Order,$r7_Uid,$r8_MP,$r9_BType)
{
    ...
    logstr($r6_Order,$sbOld,HmacMd5($sbOld,config('yeepay.merchant_key')));
    return HmacMd5($sbOld,config('yeepay.merchant_key'));
...
{
    $james=fopen(config('yeepay.log_name'),"a+");
