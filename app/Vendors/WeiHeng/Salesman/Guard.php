...
class Guard extends BaseGuard
{
    ...
    protected function fireAttemptEvent(array $credentials, $remember, $login)
    {
        if ($this->events) {
            ...
            $this->events->fire('salesman.auth.attempt', $payload);
        }
    }
    ...
    {
        if ($this->events) {
            $this->events->listen('salesman.auth.attempt', $callback);
        }
    }
    ...
    protected function fireLoginEvent($user, $remember = false)
    {
        if (isset($this->events)) {
            $this->events->fire('salesman.auth.login', [$user, $remember]);
        }
    }
    ...
    public function logout()
    {
        ...
        if (isset($this->events)) {
            $this->events->fire('salesman.auth.logout', [$user]);
        }
