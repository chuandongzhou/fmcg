var addressData = {
    ...
    "441901": ["\u839e\u57ce\u533a", "441900", "guan cheng qu"],
    "441902": ["\u5e38\u5e73\u9547", "441900", "chang ping zhen"],
    "441903": ["\u5858\u53a6\u9547", "441900", "tang xia zhen"],
    "441904": ["\u4e07\u6c5f\u533a", "441900", "wan jiang qu"],
    "441905": ["\u864e\u95e8\u9547", "441900", "hu men zhen"],
...
function Address(provinceObj, cityObj, districtObj, streetObj) {
    ...
    this.optionHtml = function (pid) {
        ...
        $.each(addressData, function (id, address) {
            if (address[1] == pid) {
                html += '<option value="' + id + '">' + address[0] + '</option>';
            }
        });
        return html;
    };
    ...
    this.updateSource = function (obj, pid, selected, prefix) {
        ...
        prefix = prefix || '';
        selected = selected || '';
        obj.html(prefix + this.optionHtml(pid));
        if (obj.find('option[value="' + selected + '"]').length) {
            obj.val(selected);
        }
    };
    this.updateSourceForStreet = function (obj, pid, selected, prefix) {
        ...
        $.post(site.api('address/street'), {pid: pid}, function (data) {
            if (data) {
                ...
                html ? obj.html(prefix + html).show() : obj.html(prefix).hide();
            }
        }, 'json');
    ...
    this.updateProvinceSource = function () {
        ...
        this.updateSource(this.province, 1, this['provinceVal'], '<option value="" selected="selected">请选择省市/其他...</option>');
    ...
    this.updateCitySource = function () {
        ...
        pid && html == prefix ? city.hide() : city.show();
        !html && city.html(prefix);
    ...
    this.updateDistrictSource = function () {
        ...
        this.updateSource(district, cid, this['districtVal'], prefix);

        var html = district.html();

        cid && html == prefix ? district.hide() : district.show();
        !html && district.html(prefix);
        this.updateStreetSource();
    };
    // 更新地区数据
    this.updateStreetSource = function () {
        var street = this.street,
            prefix = '<option value="" selected="selected">请选择街道...</option>';

        if (!this.district || !street) {
            return;
        }
...
Address.prototype = {
    ...
    setProvinceObject: function (obj, val) {
        ...
        // 移除旧的绑定事件
        if (this.province) {
            this.province.off('change', '', this.updateCitySource);
        }

        (val === undefined) && (val = obj.data('id'));
        return this.setObject('province', obj, val).on('change', '', function () {
    ...
    setCityObject: function (obj, val) {
        ...
        // 移除旧的绑定事件
        if (this.city) {
            this.city.off('change', '', this.updateDistrictSource);
        }
    ...
    setDistrictObject: function (obj, val) {
        ...
        // 移除旧的绑定事件
        if (this.district) {
            this.district.off('change', '', this.updateStreetSource);
        }
