...
function Address(provinceObj, cityObj) {
     ...
     * @returns {string}
     */
    this.optionHtml = function (pid) {
        var html = '';

        $.each(addressData, function (id, address) {
            if (address[1] == pid) {
                html += '<div class="item"><a href="javascript:void(0)" data-id="' + id + '">' + address[0] + '</a></div>';
            }
        });
        return html;
    };
     ...
     * @returns {object}
     */
    this.setObject = function (name, obj, val) {
        var obj = $(obj).first();
        // 更新内部变量
        if (obj.length) {
            this[name] = obj;
            this[name + 'Val'] = val;
            // 首字母大写
            this['update' + name.charAt(0).toUpperCase() + name.slice(1) + 'Source']();
        } else {
            this[name] = undefined;
     ...
     * @param {string} [prefix]
     */
    this.updateSource = function (obj, pid, selected, prefix) {
        if (!obj.length) {
            return;
        }
        prefix = prefix || '';
        selected = selected || '';
        var addressPanel = $('.' + obj.data('class'))
        addressPanel.html(prefix + this.optionHtml(pid));
        var selectPanel = addressPanel.find('a[data-id="' + selected + '"]');

        if (selectPanel.length) {
            obj.data('id', selected).html(selectPanel.html());
        } else {
            var addressName = obj.parent().index() == 0 ? addressPanel.find('a[data-id="' + defaultProvinceId + '"]').html() : addressPanel.find('div').first().find('a').html();
            obj.data('id', defaultProvinceId).html(addressName);
        }
    };
    /**
     * protect 更新省份数据
     */
    this.updateProvinceSource = function () {
        if (!this.province) {
            return;
        }
        this.updateSource(this.province, 1, this['provinceVal']);
        this.updateCitySource();
    };

    /**
     * protect 更新城市数据
     */
    this.updateCitySource = function () {
        var city = this.city;
        if (!this.province || !city) {
            return;
        }

        var pid = this.province.data('id');

        this.updateSource(city, pid, this['cityVal']);
    };
...
Address.prototype = {
     ...
     * @returns {object}
     */
    setProvinceObject: function (obj, val) {
        var that = this;

        // 移除旧的绑定事件
        if (this.province) {
            this.province.off('click', '', this.updateCitySource);
        }

        (val === undefined) && (val = obj.data('id'));

        return this.setObject('province', obj, val).on('click', '', function () {
            var obj = $(this), addressPanel = $('.' + obj.data('class')), deliveryCity = obj.parent().next();
            addressPanel.addClass('in').show().siblings().removeClass('in').hide();
            obj.html('请选择');
            deliveryCity.hide();
            addressPanel.on('click', '.item a', function () {
                var target = $(this), dataId = target.data('id'), provinceName = target.html();
                obj.data('id', dataId).html(provinceName);
                that.updateCitySource();
                deliveryCity.addClass('active').show().siblings().removeClass('active');
                addressPanel.removeClass('in').hide().siblings().addClass('in').show();
            })
        });
    },
    ...
    setCityObject: function (obj, val) {
        (val === undefined) && (val = obj.data('id'));
        this.setObject('city', obj, val);

        $('.' + obj.data('class')).on('click', '.item a', function () {
            var target = $(this), cityId = target.data('id'), provinceId = obj.parent().prev().children('a').data('id');
            obj.data('id', cityId).html(target.html());
...
!function ($) {
    ...
    // 初始化操作
    var addressGroup = {};
    $('.delivery-province, .delivery-city').each(function () {
    ...
    $.each(addressGroup, function (group, addresses) {
        var province = addresses.filter('.delivery-province'),
            city = addresses.filter('.delivery-city');

        if (province.length && city.length) {
            var address = new Address(province, city);
