    ...
    <div class="col-sm-10">
        <div class="row controls">
            <div class="col-sm-12">
                <div class="item control-sequence">
                    <label>排序:</label>
                    <a href="{{ url('goods?sort=name'  . (empty(array_except($get , ['sort'])) ? '' :  '&' . http_build_query(array_except($get , ['sort'])))) }}"
                       class="btn {{ !isset($get['sort']) || $get['sort'] == 'name' ? 'btn-primary' : 'btn-default' }} control">名称</a>
                    <a href="{{ url('goods?sort=price'  . (empty(array_except($get , ['sort'])) ? '' :  '&' . http_build_query(array_except($get , ['sort'])))) }}"
                       class="btn  control {{ isset($get['sort']) && $get['sort'] == 'price' ? 'btn-primary' : 'btn-default' }}">价格</a>
                    <a href="{{ url('goods?sort=new'  . (empty(array_except($get , ['sort'])) ? '' :  '&' . http_build_query(array_except($get , ['sort'])))) }}"
        ...
        <div class="row sort">
            <div class="col-sm-12 sort-panel">
                <p class="sort-item">
                    <label>分类 : </label>
                    <a href="{{ url('goods' . (empty(array_except($data , ['category_id' , 'attr'])) ? '' :  '?' . http_build_query(array_except($data , ['category_id' , 'attr']))) )}}"
                       class="btn  control {{ !isset($get['category_id']) ? 'btn-primary' : '' }}">
                        全部
                    </a>
                    @foreach($categories as $key => $category)
                        <a href="{{ url('goods?category_id=' . $category['level'].$category['id'] . (empty(array_except($data , ['category_id' , 'attr'])) ? '' :  '&' . http_build_query(array_except($data , ['category_id' , 'attr'])))) }}"
                ...
                @foreach($attrs as $attr)
                    <p class="sort-item package">
                        <label>{{ $attr['name'] }} : </label>
                        <a href="{{ url('goods'. (empty(array_except($get , ['attr_' . $attr['id']])) ? '' : '?'. http_build_query(array_except($get , ['attr_' . $attr['id']])))) }}"
                           class="btn  control {{ !isset($get['attr_' . $attr['id']]) ? 'btn-primary' : '' }}">
                            全部
                        </a>
                        @foreach($attr['child'] as $child)
                            <a href="{{ url('goods?attr_' . $attr['id'] . '=' . $child['id']  . '&' . http_build_query(array_except($get , ['attr_' . $attr['id']]))) }}"
