        ...
        <div class="row">
            <div class="col-sm-12 ">
                <div class="tab-title clearfix">
                    <p class="sequence">
                        <a class="{{ $type == 'all' || !$type  ? 'active' : '' }}"
                           href="{{ url('shop/' . $shop->id) }}">全部</a>
                        <a class="{{ $type == 'hot' ? 'active' : '' }}"
                           href="{{ url('shop/'.$shop->id . '/hot') }}">热销</a>
                        <a class="{{ $type == 'new' ? 'active' : '' }}"
                           href="{{url('shop/'.$shop->id .  '/new') }}">最新</a>
                        <a class="{{ $type == 'promotion' ? 'active' : '' }}"
        ...
        <div class="row list-penal">
            @foreach($goods as $item)
                <div class="col-sm-3 commodity">
                    <div class="img-wrap">
                        <a href="{{ url('my-goods/' . $item->id) }}"><img class="commodity-img"
                    ...
                    <p class="commodity-name"><a href="{{ url('my-goods/' . $item->id) }}">{{ $item->name }}</a></p>
