    ...
    <div class="row choice-template margin-clear">
        <div class="col-sm-12 ">
            <div class="row template-item">
                <div class="col-sm-12 title">
                    ...
                    <div class="pull-right">
                        <a class="btn check-index" href="{{ url('shop/'.$shop->id)  }}" target="_blank">查看店铺首页</a>
                    </div>
                </div>
                <div class="col-sm-12 item-wrap">
                    <form class="form-horizontal ajax-form" action="{{ url('api/v1/personal/model/signature') }}"
                          method="post"
                          data-help-class="col-sm-push-2 col-sm-10"
                          data-done-url="{{ url('personal/model/model-edit') }}"
                          autocomplete="off">

                        <input type="hidden" name="id" value="{{ !empty($shop->ShopSignature)?$shop->ShopSignature->id:''}}" />
                        <div class="row">
                            <div class="col-sm-12 item upload-banner-item">上传店招
                                ...
                                <div class="shop-img  image-preview" id="shop-img" >
                                    ...
                                    <input id="text-input" style="color:{{ $shop->ShopSignature?$shop->ShopSignature->color:'#000000' }}" name="text" type="text" placeholder="可编辑文字" value="{{ $shop->ShopSignature?$shop->ShopSignature->text:'' }}" autofocus="autofocus"  maxlength="30"/>
                                </div>
                            </div>
            ...
            <div class="row template-item">
                ...
                <div class="col-sm-12 item-wrap">
                    <form class="form-horizontal ajax-form" action="{{ url('api/v1/personal/model/shop-advert') }}"
                          method="post"
                          data-help-class="col-sm-push-2 col-sm-10"
                          data-done-url="{{ url('personal/model/model-edit') }}"
                          autocomplete="off">
                            ...
                            @if(!empty($shop->shopHomeAdverts))
                                @foreach($shop->shopHomeAdverts as $key => $shopHomeAdvert)
                                    <div class="col-sm-3 item upload-banner-item">
                                        ...
                                        <div class="option">
                                            <input type="text" name="images[url][]" value="{{ $shopHomeAdvert->url }}"
                                                   placeholder="输入图片链接"/>
                                        </div>
                                        <div class="option">
                                            <label>
                                    ...
                                    <div class="col-sm-3 item upload-banner-item">
                                        ...
                                        <div class="option">
                                            <label>
                                                ...
                                                <input class="status" type="hidden" name="images[status][]" value="0"/>
                                                显示</label>
                                        </div>
                                    </div>
                                @endfor
