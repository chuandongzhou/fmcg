...
@section('top-title')
    <a href="{{ url('child-user/info') }}">个人中心</a> > <span class="second-level"> 配送历史</span>
@stop
        ...
        <div class="page-content">
            <div class="row">
                <div class="col-sm-12 path-title">
                    <a href="{{ url('child-user/delivery-man') }}">配送管理</a> > <span class="second-level"> 配送历史</span>
                </div>
            </div>
                        ...
                        @foreach($deliveries as $delivery)
                            <tr align="center">
                                @if($delivery->deliveryMan)
                                    <td>{!!  $delivery->deliveryMan->pluck('name')->implode('|') !!} </td>
                                @else
                                    <td></td>
                                @endif
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        $('.statistical').click(function () {
            checksubmit(site.url('child-user/delivery/statistical'));
        });
        $('.search-by-get').click(function () {
            checksubmit(site.url('child-user/delivery'));
        });
