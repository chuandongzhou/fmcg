@section('body')
    <div class="modal fade" id="warehouseKeeperModal" tabindex="-1" role="dialog" aria-labelledby="warehouseKeeperModalLabel"
         aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form class="form-horizontal ajax-form" action="{{ url('api/v1/warehouse-keeper') }}"
                    ...
                    <div class="modal-body address-select">
                        ...
                        <div class="form-group row">
                            <label class="col-sm-2 control-label" for="user_name">账号:</label>
...
@section('js')
    ...
    <script type="text/javascript">
        $(function () {
            var warehouseKeeperModal = $('#warehouseKeeperModal'),
                form = warehouseKeeperModal.find('form');
            warehouseKeeperModal.on('shown.bs.modal', function (e) {
                var obj = $(e.relatedTarget);
                $('#warehouseKeeperModalLabel span').html(obj.hasClass('add') ? '添加仓管员' : '编辑仓管员');
                $('.btn-add').html('提交');
                var id = obj.data('id') || '',
                    name = obj.data('name') || '',
                ...
                form.attr('action', site.api(obj.hasClass('add') ? 'warehouse-keeper' : 'warehouse-keeper/' + id));
